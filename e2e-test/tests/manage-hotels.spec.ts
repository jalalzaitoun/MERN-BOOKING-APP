...
test("should allow user to add a hotel", async ({ page }) => {
  ...
  await page.setInputFiles('[name="imageFiles"]', [
    ...
    // path.join(__dirname, "files", "2.jpg"), مشان خفف حمل (ضعف نت )
  ]);
...
test("should edit hotel", async ({ page }) => {
  ...
  await expect(page.locator('[name="name"]')).toHaveValue("Aleppo citadel");
  await page.locator('[name="name"]').fill("Aleppo citadel update");
  await page.getByRole("button", { name: "Save" }).click();
  await expect(page.getByText("Hotel Saved!")).toBeVisible();
  ...
  await expect(page.locator('[name="name"]')).toHaveValue(
    "Aleppo citadel update"
  );
  await page.locator('[name="name"]').fill("Aleppo citadel");
  await page.getByRole("button", { name: "Save" }).click();
});
